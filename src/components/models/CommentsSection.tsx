...
import { getCommentsByModel } from '@/lib/mock-data'
...
export default function CommentsSection({ modelId }: CommentsSectionProps) {
  ...
  useEffect(() => {
    setComments(getCommentsByModel(modelId))
  }, [modelId])

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!user) { showToast('Faça login para comentar.', 'error'); return }
              ...
              <div key={comment.id} className="flex gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-medium text-gray-900">{comment.profile?.name || 'Usuário'}</span>
                    <span className="text-xs text-gray-400">{formatDate(comment.created_at)}</span>
                  </div>
                  {editingId === comment.id ? (
                    <div>
                      <Textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="mb-2 min-h-[60px] text-sm" />
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {(isOwner || isAdmin) && (
                        <div className="flex gap-1 shrink-0">
                          {isOwner && (
                            <button onClick={() => { setEditingId(comment.id); setEditContent(comment.content) }} className="text-gray-400 hover:text-blue-600 transition-colors" title="Editar">
                              <Edit2 size={13} />
                            </button>
                          )}
                          <button onClick={() => handleDelete(comment.id)} className="text-gray-400 hover:text-red-600 transition-colors" title="Excluir">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      )}
